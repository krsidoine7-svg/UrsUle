...
export const statsService = {
  ...
  async getTodayStats(userId: string) {
    try {
      ...
      console.error('Error in statsService.getTodayStats:', e)
  ...
  async getIntervalStats(userId: string, from: Date, to: Date) {
    try {
      ...
      return days.map(day => {
        const created = tasks?.filter((t: any) => isValidDate(t.created_at) && isSameDay(parseSafeDate(t.created_at), day)).length || 0
        const completed = tasks?.filter((t: any) => t.completed_at && isValidDate(t.completed_at) && isSameDay(parseSafeDate(t.completed_at), day)).length || 0
        return {
      ...
      console.error('Error in statsService.getIntervalStats:', e)
      return []
    }
  },
  ...
  async getCategoryDistribution(userId: string, from: Date, to: Date) {
    try {
      ...
      console.error('Error in statsService.getCategoryDistribution:', e)
      return []
    }
  },
  ...
  async getMoodDistribution(userId: string, from: Date, to: Date) {
    try {
      ...
      console.error('Error in statsService.getMoodDistribution:', e)
      return []
    }
  },
  ...
  async getHeatmapData(userId: string) {
    try {
      ...
      tasks?.forEach((t: any) => {
        if (t.completed_at && isValidDate(t.completed_at)) {
          const date = format(parseSafeDate(t.completed_at), 'yyyy-MM-dd')
      ...
      console.error('Error in statsService.getHeatmapData:', e)
      return {}
    }
  },
  ...
  async getTotalTime(userId: string, from: Date, to: Date) {
    try {
      ...
      console.error('Error in statsService.getTotalTime:', e)
      return 0
    }
  }
}
